...
public:
    ...
    // Called by SoLoud to shut down the back-end. If NULL, not called. Should
    // be set by back-end.
    ...
    handle play3d(AudioSource& aSound, float aPosX, float aPosY, float aPosZ,
                  ...
                  unsigned int aBus = 0);
    // Start playing a 3d audio source, delayed in relation to other sounds
    // called via this function.
    ...
    void oscillateVolume(handle aVoiceHandle, float aFrom, float aTo,
                         time aTime);
    ...
    // Returns mixed float samples in buffer. Called by the back-end, or user
    // with null driver.
...
public:
    ...
    void postinit(unsigned int aSamplerate, unsigned int aBufferSize,
    ...
    void mixBus(float* aBuffer, unsigned int aSamples, float* aScratch,
    ...
    void clip(AlignedFloatBuffer& aBuffer, AlignedFloatBuffer& aDestBuffer,
    ...
    unsigned int mDeviceHandle;

    // Remove all non-active voices from group
    void trimVoiceGroup(handle aVoiceGroupHandle);
    // Get pointer to the zero-terminated array of voice handles in a voice
    // group
