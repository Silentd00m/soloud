...
#include <float.h>
    ...
    !defined(WITH_SDL_STATIC) && !defined(WITH_SDL2_STATIC) &&                 \
                    ...
                    unsigned int aBufferSize, unsigned int aChannels)
{
    ...
    if(aBackend == Soloud::SDL || aBackend == Soloud::AUTO) {
        if(aBufferSize == Soloud::AUTO)
            buffersize = 2048;

        int ret =
            sdlstatic_init(this, aFlags, samplerate, buffersize, aChannels);
        if(ret == 0) {
            inited     = 1;
    ...
    if(aBackend == Soloud::SDL2 || aBackend == Soloud::AUTO) {
        if(aBufferSize == Soloud::AUTO)
            buffersize = 2048;

        int ret = sdl2static_init(this, aDevice, aFlags, samplerate, buffersize,
                                  aChannels);
        if(ret == 0) {
            inited     = 1;
    ...
    if(aBackend == Soloud::SDL || aBackend == Soloud::SDL2 ||
    ...
    if(aBackend == Soloud::VITA_HOMEBREW || aBackend == Soloud::AUTO) {
        int ret =
...
void Soloud::postinit(unsigned int aSamplerate, unsigned int aBufferSize,
                      unsigned int aFlags, unsigned int aChannels)
{
    ...
    switch(mChannels) {
        ...
        case 4:
            ...
            m3dSpeakerPosition[1 * 3 + 1] = 0;
            m3dSpeakerPosition[1 * 3 + 2] = 1;
            // I suppose technically the second pair should be straight left &
            // right, but I prefer moving them a bit back to mirror the front
            // speakers.
        ...
        case 6:
            ...
            m3dSpeakerPosition[2 * 3 + 1] = 0;
            m3dSpeakerPosition[2 * 3 + 2] = 1;
            // Sub should be "mix of everything". We'll handle it as a special
            // case and make it a null vector.
...
#ifdef SOLOUD_SSE_INTRINSICS
void Soloud::clip(AlignedFloatBuffer& aBuffer, AlignedFloatBuffer& aDestBuffer,
                  unsigned int aSamples, float aVolume0, float aVolume1)
{
...
void resample(float* aSrc, float* aSrc1, float* aDst, int aSrcOffset,
              int aDstSampleCount, float aSrcSamplerate, float aDstSamplerate,
              int aStepFixed)
{
#if 0
...
void Soloud::mixBus(float* aBuffer, unsigned int aSamples, float* aScratch,
           ...
           !(voice->mFlags & AudioSourceInstance::INAUDIBLE)) {
            ...
            while(step_fixed != 0 && outofs < aSamples) {
                if(voice->mLeftoverSamples == 0) {
                    ...
                    if(voice->hasEnded()) {
                        memset(voice->mResampleData[0]->mBuffer, 0,
                               sizeof(float) * SAMPLE_GRANULARITY *
                                   voice->mChannels);
                    } else {
                        voice->getAudio(voice->mResampleData[0]->mBuffer,
                                        SAMPLE_GRANULARITY);
                    }

                    // If we go past zero, crop to zero (a bit of a kludge)
                    if(voice->mSrcOffset <
                       SAMPLE_GRANULARITY * FIXPOINT_FRAC_MUL) {
                        voice->mSrcOffset = 0;
                    } else {
                ...
                // Figure out how many samples we can generate from this source
                // data. The value may be zero.

                unsigned int writesamples = 0;

                if(voice->mSrcOffset < SAMPLE_GRANULARITY * FIXPOINT_FRAC_MUL) {
                    writesamples = ((SAMPLE_GRANULARITY * FIXPOINT_FRAC_MUL) -
                                    voice->mSrcOffset) /
                                       step_fixed +
                                   1;
                ...
                // If this is too much for our output buffer, don't write that
                // many:
                if(writesamples + outofs > aSamples) {
                    voice->mLeftoverSamples =
                        (writesamples + outofs) - aSamples;
                ...
                if(writesamples) {
                    for(j = 0; j < voice->mChannels; j++) {
                        resample(voice->mResampleData[0]->mBuffer +
                                     SAMPLE_GRANULARITY * j,
                                 voice->mResampleData[1]->mBuffer +
                                     SAMPLE_GRANULARITY * j,
                                 aScratch + aSamples * j + outofs,
                                 voice->mSrcOffset, writesamples,
                                 voice->mSamplerate, aSamplerate, step_fixed);
                    }
                }
            ...
            int ofs = 0;
            switch(aChannels) {
                case 1: // Target is mono. Sum everything. (1->1, 2->1, 4->1,
                        // 6->1)
                    for(j = 0, ofs = 0; j < voice->mChannels;
                        j++, ofs += aSamples) {
                ...
                case 2:
                    switch(voice->mChannels) {
                        case 6: // 6->2, just sum lefties and righties, add a
                                // bit of center, ignore sub?
                    ...
                    switch(voice->mChannels) {
                        ...
                        case 4: // 4->6
                            for(j = 0; j < aSamples; j++) {
                                ...
                                aBuffer[j + 0] += s1 * pan[0];
                                aBuffer[j + aSamples] += s2 * pan[1];
                                aBuffer[j + aSamples * 2] +=
                                    0.5f * (s1 + s2) * pan[2];
                                aBuffer[j + aSamples * 3] +=
            ...
            // Inaudible but needs ticking. Do minimal work (keep counters up to
            // date and ask audiosource for data)
            float        step       = voice->mSamplerate / aSamplerate;
...
void Soloud::calcActiveVoices()
{
    ...
    // time. It is a must when new voices are started, but otherwise we could
    // get away with postponing it sometimes..
    ...
    if(mustlive >= mMaxActiveVoices) {
        ...
        // This is a potentially an error situation, but we have no way to
        // report error from here. And asserting could be bad, too.
        return;
    }

    // If we get this far, there's nothing to it: we'll have to sort the voices
    // to find the most audible.
...
void Soloud::mix_internal(unsigned int aSamples)
{
    ...
    for(i = 0; i < (signed)mHighestVoice; i++) {
        if(mVoice[i] && !(mVoice[i]->mFlags & AudioSourceInstance::PAUSED)) {
            ...
            if(mVoice[i]->mRelativePlaySpeedFader.mActive > 0) {
                float speed = mVoice[i]->mRelativePlaySpeedFader.get(
                    mVoice[i]->mStreamTime);
    ...
    mixBus(mOutputScratch.mData, aSamples, mScratch.mData, 0,
           (float)mSamplerate, mChannels);

    for(i = 0; i < FILTERS_PER_STREAM; i++) {
        if(mFilterInstance[i]) {
            mFilterInstance[i]->filter(mOutputScratch.mData, aSamples,
                                       mChannels, (float)mSamplerate,
                                       mStreamTime);
        }
    }
        ...
        if(aSamples > 255) {
            for(i = 0; i < 256; i++) {
                ...
                for(j = 0; j < (signed)mChannels; j++) {
                    mVisualizationWaveData[i] +=
                        mScratch.mData[i + j * aSamples];
                }
            }
        } else {
            ...
            for(i = 0; i < 256; i++) {
                ...
                for(j = 0; j < (signed)mChannels; j++) {
                    mVisualizationWaveData[i] +=
                        mScratch.mData[(i % aSamples) + j * aSamples];
                }
            }
        }
    }
